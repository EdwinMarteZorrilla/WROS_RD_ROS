...
from rrclite_sdk import Board  # Buzzer/LED SDK
...
class FireFighterRobot:
    def __init__(self):
        ...
        # Initialize Board for buzzer/LED
        self.board = Board(device="/dev/ttyACM0")
    ...
    def navigation_state(self):
        try:
            ...
            # Start buzzer signaling (non-blocking)
            self.board.set_buzzer(freq=1500, on_time=0.3, off_time=0.1, repeat=0)  # 0 = continuous until stopped

            # Run navigation script
            subprocess.run(["python3", NAVIGATION_SCRIPT, str(self.last_rpi)], check=True)

            # Stop buzzer after navigation ends
            self.board.set_buzzer(freq=0, on_time=0, off_time=0, repeat=0)
    ...
    def search_state(self):
        try:
            print("[STATE] 🎥 SEARCH: Moving servo camera (no detection).")
