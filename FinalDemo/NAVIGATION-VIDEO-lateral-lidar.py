...
class IMUReader(Node):
    ...
    def imu_callback(self, msg):
        ...
        # --- smooth yaw to reduce IMU noise ---
        self.yaw = 0.8 * self.yaw + 0.2 * new_yaw
...
class OdometryReader(Node):
    ...
    def odom_callback(self, msg):
        ...
        # --- adaptive IMU/odom yaw fusion ---
        try:
            vx = msg.twist.twist.linear.x
            vy = msg.twist.twist.linear.y
            v_mag = math.hypot(vx, vy)
            lateral_ratio = abs(vy) / (v_mag + 1e-6)
        except AttributeError:
            lateral_ratio = 0.0

        # 0.95 forward, 0.6 lateral
        alpha = 0.95 - 0.35 * lateral_ratio
...
class LidarReader(Node):
    ...
    def __init__(self, topic='/scan_raw', stop_distance=0.10):
...
class CmdVelPublisher(Node):
    ...
    def move_direction(self, dr, dc, odom_sub, distance=GRID_SIZE, speed=0.4):
        ...
        # --- calibration factors ---
        VX_SCALE = 1.0
        VY_SCALE = 0.85  # tune this if lateral distance under/overshoots
        vx_map_unit *= VX_SCALE
        vy_map_unit *= VY_SCALE
        ...
        while rclpy.ok():
            ...
            angular_z = k_yaw * yaw_err
            if abs(dr) < 1e-3 and abs(dc) > 1e-3:
                angular_z *= 0.3
            ...
            dir_x = vx_map / mag
            dir_y = vy_map / mag
            cross_err = (-dir_y * dx) + (dir_x * dy)
            vy_map_unit_corr = vy_map_unit - k_lat * cross_err

            # --- transform to body frame ---
            cos_yaw = math.cos(fused_yaw)
            sin_yaw = math.sin(fused_yaw)
            linear_x_body =  vx_map_unit * cos_yaw + vy_map_unit_corr * sin_yaw
            linear_y_body = -vx_map_unit * sin_yaw + vy_map_unit_corr * cos_yaw

            print(f"[move_dir] fused_yaw={fused_yaw:+.2f}, yaw_err={yaw_err:+.2f}, "
                  f"cross_err={cross_err:+.3f}, vx_body={linear_x_body:+.2f}, "
                  f"vy_body={linear_y_body:+.2f}, ang_z={angular_z:+.2f}")
...
def parse_map(layout):
    ...
    if start is None or goal is None:
        raise ValueError("Map must contain 'R' and 'G'")
...
def follow_path(node, path, odom_sub, imu_sub, maze, start, goal, lidar_sub):    
    ...
    while i<len(path):
        ...
        # while i+run_len<len(path) and \
            ...
            # run_len+=1
        # total=GRID_SIZE*run_len
        # rclpy.spin_once(imu_sub)
        # odom_sub.imu_yaw=imu_sub.yaw
        # node.move_direction(dr,dc,odom_sub,distance=total)
        # i+=run_len
        # rclpy.spin_once(odom_sub)
        # plot_maze(maze,start,goal,path,robot_pos=(odom_sub.x_pos,odom_sub.y_pos))
        while distance_traveled < total and rclpy.ok():
            rclpy.spin_once(lidar_sub)
            if lidar_sub.obstacle_detected():
                print(f"⚠️ Obstacle detected at {lidar_sub.front_distance*100:.1f} cm — stopping.")
                node.stop()
                # --- Replan from current odom position ---
                cur_pos = (round(-odom_sub.y_pos / GRID_SIZE),
                           round(odom_sub.x_pos / GRID_SIZE))
                print(f"Replanning path from {cur_pos} to {goal}...")
                path = bfs_path(maze, cur_pos, goal)
                i = 1  # restart following new path
                break

            node.move_direction(dr, dc, odom_sub, distance=GRID_SIZE)
            rclpy.spin_once(odom_sub)
            dx = odom_sub.x_pos - start_x
            dy = odom_sub.y_pos - start_y
...
def main():
    ...
    lidar_reader = LidarReader()   # ✅ new lidar node
    ...
    target_map_yaw=ORIENTATION_TO_YAW["west"]
    ...
    print(f"Aligning: fused_yaw={current_fused:.3f}, target={target_map_yaw:.3f}, offset={odom_reader.map_yaw_offset:.3f}")
